...
try {
    ...
    error_log("Dashboard carregado para user_id: " . ($_SESSION['user_id'] ?? 'não definido'));
    error_log("Username da sessão: " . ($_SESSION['username'] ?? 'não definido'));
    
    // Debug: Test if user exists in database
    $userCheckStmt = $conn->prepare("SELECT id, username FROM users WHERE id = ?");
    $userCheckStmt->execute([$_SESSION['user_id']]);
    $userExists = $userCheckStmt->fetch();
    error_log("Usuário encontrado no banco: " . ($userExists ? "SIM (ID: " . $userExists['id'] . ", Username: " . $userExists['username'] . ")" : "NÃO"));
    ...
    // Get total count separately for better compatibility
    if (!empty($pokemon_collection)) {
        $total_pokemon = count($pokemon_collection);
    }
    
    // Debug: Log the exact query and result
    error_log("Query executada: SELECT pc.* FROM pokemon_collection pc WHERE pc.user_id = " . $_SESSION['user_id'] . " ORDER BY pc.created_at DESC");
    error_log("Resultado da query: " . count($pokemon_collection) . " registros encontrados");
    
    if (!empty($pokemon_collection)) {
        $total_pokemon = count($pokemon_collection);
        error_log("Encontrados " . count($pokemon_collection) . " Pokémon na coleção do usuário");
        // Debug: Log first pokemon details
        error_log("Primeiro Pokémon: " . print_r($pokemon_collection[0], true));
    } else {
        error_log("Nenhum Pokémon encontrado na coleção do usuário");
        
        // Debug: Check if there are ANY pokemon in the table
        $allPokemonStmt = $conn->query("SELECT COUNT(*) as total FROM pokemon_collection");
        $totalInTable = $allPokemonStmt->fetch();
        error_log("Total de Pokémon na tabela (todos os usuários): " . $totalInTable['total']);
        
        // Debug: Check if there are pokemon for this specific user with a simpler query
        $simpleStmt = $conn->prepare("SELECT id, pokemon_name, user_id FROM pokemon_collection WHERE user_id = ?");
        $simpleStmt->execute([$_SESSION['user_id']]);
        $simplePokemon = $simpleStmt->fetchAll();
        error_log("Query simples encontrou: " . count($simplePokemon) . " Pokémon para user_id " . $_SESSION['user_id']);
        if (!empty($simplePokemon)) {
            error_log("Detalhes da query simples: " . print_r($simplePokemon, true));
        }
    }
    ...
    <div class="container dashboard-container">
            ...
            <div class="stats">
                ...
                <div class="stat-card">
                    <div class="stat-number"><?php 
                        if (!empty($pokemon_collection)) {
                            $types = array_column($pokemon_collection, 'pokemon_type1');
                            echo count(array_unique(array_filter($types)));
                        } else {
                            echo 0;
                        }
                    ?></div>
        ...
        <div class="dashboard-actions">
            ...
            <div class="search-filter">
                <input type="text" id="search-pokemon" placeholder="Buscar na sua coleção..." autocomplete="off">
            </div>
        </div>
                ...
                <div class="modal-body">
                    <form id="search-form" autocomplete="off">
                        <div class="form-group">
                            ...
                            <input type="text" id="pokemon-search" placeholder="ex: pikachu ou 25" autocomplete="off">
                    ...
                    <form id="add-pokemon-form" style="display: none;" autocomplete="off">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" id="level_caught" name="level_caught" min="1" max="100" value="1" autocomplete="off">
                        </div>
                        ...
                        <button type="submit" id="add-pokemon-submit" class="btn btn-primary">Adicionar à Coleção</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div id="edit-pokemon-modal" class="modal">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="edit-pokemon-form" autocomplete="off">
                        <input type="hidden" id="edit-id" name="id">
                        
                        <div class="form-group">
                            <label for="edit-nickname">Apelido:</label>
                            <input type="text" id="edit-nickname" name="nickname" autocomplete="off">
                        </div>
                        
                        <div class="form-group">
                            <label for="edit-level">Nível:</label>
                            <input type="number" id="edit-level" name="level_caught" min="1" max="100" autocomplete="off">
                        </div>
                        
                        <div class="form-group">
                            <label for="edit-notes">Anotações:</label>
                            <textarea id="edit-notes" name="notes" rows="3" autocomplete="off"></textarea>
                        </div>
                    ...
                    <?php foreach ($pokemon_collection as $pokemon): ?>
                        <div class="pokemon-card" data-pokemon-id="<?php echo $pokemon['pokemon_id']; ?>" data-name="<?php echo strtolower($pokemon['pokemon_name']); ?>">
                            ...
                            <div class="pokemon-info">
                                ...
                                <div class="pokemon-date">Capturado em: <?php echo date('j/m/Y', strtotime($pokemon['date_caught'])); ?></div>
                                <?php if (!empty($pokemon['notes'])): ?>
                                    <div class="pokemon-notes">
                                        <strong>Anotações:</strong> <?php echo htmlspecialchars($pokemon['notes']); ?>
                                    </div>
                                <?php endif; ?>
                            </div>
                            <div class="pokemon-actions">
                                <button class="btn btn-sm btn-outline edit-pokemon-btn" data-id="<?php echo $pokemon['id']; ?>">Editar</button>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
